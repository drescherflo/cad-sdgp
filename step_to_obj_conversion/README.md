...
### Konvertierung von STEP-Dateien in OBJ-Dateien

Das Skript `step_to_obj.py` ermöglicht die Konvertierung von 3D-Modellen im STEP-Format (`.stp`) in das OBJ-Format (`.obj`). 
...
3. **Führen Sie das Skript aus:** Verwenden Sie den folgenden Befehl, wobei Sie die Pfade zu den Verzeichnissen entsprechend anpassen:
   ```bash
   python step_to_obj.py --input_dir [Pfad_zum_Eingabe-Verzeichnis] --output_dir [Pfad_zum_Ausgabe-Verzeichnis]
   ```
   - `--input_dir`: Pfad zum Verzeichnis, das die `.stp`-Dateien enthält.
   - `--output_dir`: Pfad zum Verzeichnis, in das die konvertierten `.obj`-Dateien gespeichert werden sollen.

#### Beispiel:

```
python step_to_obj.py --input_dir ~/STEP --output_dir ~/OBJ
```

#### Wichtige Hinweise:

- Die Bibliothek `pythonocc-core` kann nur mittels Anaconda installiert werden.
- Das Skript liest alle `.stp` und `.step`-Dateien aus dem angegebenen Eingabeverzeichnis und speichert die konvertierten `.obj`-Dateien im Ausgabeverzeichnis.
...
```
  ~/.local/share/ov/pkg/isaac_sim-2023.1.1/python.sh ~/.local/share/ov/pkg/isaac_sim-2023.1.1/standalone_examples/api/omni.kit.asset_converter/asset_usd_converter.py --folders ~/OBJ
```
